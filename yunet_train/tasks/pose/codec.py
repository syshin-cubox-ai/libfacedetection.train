...
def pose_keypoints_decode(
    ...
    *,
    ...
    keypoints = keypoint_preds.reshape(
        *keypoint_preds.shape[:-1], kpt_num, kpt_dim
    ).clone()
    keypoints[..., :2] = (
        keypoints[..., :2] * priors[..., None, 2:] + priors[..., None, :2]
    )
    return keypoints
...
def pose_keypoints_encode(
    ...
    *,
    ...
    if keypoints.shape[-2:] != (kpt_num, kpt_dim):
        raise ValueError(
            f"Expected keypoints shape (..., {kpt_num}, {kpt_dim}), got {tuple(keypoints.shape)}"
        )
    encoded = keypoints.clone()
    encoded[..., :2] = (encoded[..., :2] - priors[..., None, :2]) / priors[
        ..., None, 2:
    ]
