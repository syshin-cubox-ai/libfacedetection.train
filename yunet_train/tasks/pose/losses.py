...
def oks_keypoint_loss(
    ...
    *,
    ...
    if pred_keypoints.shape[:2] != target_keypoints.shape[:2]:
        raise ValueError(
            "pred_keypoints and target_keypoints must share the first two dimensions"
        )
    if target_keypoints.shape[-1] < 3:
        raise ValueError(
            "target_keypoints must include visibility in the last dimension"
        )

    if sigmas is None:
        sigmas = torch.as_tensor(
            COCO17_OKS_SIGMA, dtype=pred_keypoints.dtype, device=pred_keypoints.device
        )
    else:
    ...
    squared_distance = (
        (pred_keypoints[..., :2] - target_keypoints[..., :2]).square().sum(dim=-1)
    )
    area = (
        target_areas.to(dtype=pred_keypoints.dtype, device=pred_keypoints.device)
        .clamp_min(eps)
        .unsqueeze(-1)
    )
