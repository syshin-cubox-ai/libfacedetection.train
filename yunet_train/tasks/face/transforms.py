...
class RandomHorizontalFlip:
    ...
    def __call__(self, sample: FaceSample) -> FaceSample:
        ...
        sample.ignored_boxes = _flip_boxes(
            _ensure_numpy_array(sample.ignored_boxes), width
        )
...
class ToTensor:
    def __call__(self, sample: FaceSample) -> FaceSample:
        ...
        sample.ignored_boxes = torch.from_numpy(
            np.ascontiguousarray(sample.ignored_boxes)
        ).float()
        sample.ignored_labels = torch.from_numpy(
            np.ascontiguousarray(sample.ignored_labels)
        ).long()
        return sample
    ...
    def __call__(self, sample: FaceSample) -> FaceSample:
        ...
        while True:
            ...
            for _ in range(250):
                ...
                sample.labels = _ensure_numpy_array(sample.labels)[keep]
                sample.keypoints = _crop_keypoints(
                    _ensure_numpy_array(sample.keypoints), patch, keep, self.clip_border
                )

                ignored_boxes = _ensure_numpy_array(sample.ignored_boxes)
                ignored_keep = _centers_in_patch(ignored_boxes, patch)
                sample.ignored_boxes = _crop_boxes(
                    ignored_boxes, patch, ignored_keep, self.clip_border
                )
                sample.ignored_labels = _ensure_numpy_array(sample.ignored_labels)[
                    ignored_keep
                ]
...
def build_eval_transforms(
    image_size: int | None = 640, *, size_divisor: int | None = None
) -> Compose:
...
def _crop_keypoints(
    keypoints: np.ndarray, patch: np.ndarray, keep: np.ndarray, clip: bool
) -> np.ndarray:
...
def _crop_image_with_padding(
    image: np.ndarray, patch: np.ndarray, crop_h: int, crop_w: int
) -> np.ndarray:
